...
void runPoint(const TString infile, const TString tag)
{
   ...
   Float_t Jet_mass[50];
   Int_t Jet_jetId[50];
   Float_t Jet_btagDeepB[50];
   Int_t Jet_hadronFlavour[50];
   Int_t Jet_partonFlavour[50];
   ...
   t->SetBranchAddress("Jet_partonFlavour", Jet_partonFlavour);
   t->SetBranchAddress("Jet_hadronFlavour", Jet_hadronFlavour);
 
   TH1D * h_nb = new TH1D("h_nb", tag+";# of b-tagged jets (tight);events / 1", 4, -0.5, 3.5);
   h_nb->SetLineWidth(2);
   h_nb->SetStats(0);
   h_nb->Sumw2();
   h_nb->SetLineColor(6);
   ...
   for (int i = 0; i < t->GetEntries(); ++i) {
      ...
      if (Muon_.DeltaR(Tau_)<0.4) continue;
      //if ((Muon_+Tau_).M()<100.) continue;

      /*int nbjet = 0;
      for (UInt_t j = 0; j < nJet; ++j) { 
         if (Jet_partonFlavour[j]==5 || Jet_hadronFlavour[j]==5) {
            if ((4&Jet_jetId[j]) && Jet_pt[j]>=20. && TMath::Abs(Jet_eta[j])<2.5) {
               ++nbjet;
            }
         }
      }
   ...
   addOverflow(h_nb);
   addOverflow(h_nb_overlap);
   h_nb->Scale(1./h_nb->Integral());
   h_nb_overlap->Scale(1./h_nb_overlap->Integral());

   for (int i = 0; i < h_nb->GetNbinsX(); ++i) {
      std::cout << i << std::endl;
      std::cout << h_nb->GetBinContent(i+1) << std::endl;
      std::cout << h_nb_overlap->GetBinContent(i+1) << std::endl;
   }
   ...
   h_nb->SetMaximum(10.);
   l->Draw();
   c->SaveAs("./plots/overlapCheck."+tag+".tight.pdf");
...
void bOverlap()
{
   ...
   const TString infile4 = "/store/mc/RunIIAutumn18NanoAODv7/ST_tW_top_5f_NoFullyHadronicDecays_TuneCP5_13TeV-powheg-pythia8/NANOAODSIM/Nano02Apr2020_EXT_102X_upgrade2018_realistic_v21_ext1-v1/20000/349D8B39-7534-6543-A78D-C121A37BD899.root";
   const TString tag4 = "ST_tW_top";
   runPoint(infile4, tag4);
}
