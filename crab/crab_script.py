...
cut_MuTau = "Sum$(TMath::Abs(Muon_eta)<2.4 && Muon_pt>=24. && Muon_tightId && Muon_pfIsoId>=2)>0 && Sum$(Tau_pt>=20. && TMath::Abs(Tau_eta)<2.3 && Tau_decayMode!=5 && Tau_decayMode!=6 && (1&Tau_idDeepTau2017v2p1VSjet) && (8&Tau_idDeepTau2017v2p1VSmu) && (4&Tau_idDeepTau2017v2p1VSe))>0"
...
#cut_ElMu = "Sum$(TMath::Abs(Electron_eta)<2.5 && Electron_pt>=23. && Electron_mvaFall17V2Iso_WP90)>0 && Sum$(TMath::Abs(Muon_eta)<2.4 && Muon_pt>=23. && Muon_tightId && Muon_pfIsoId>=2)>0"
...
#from PhysicsTools.NanoAODTools.postprocessing.examples.GenMassProducer import GenMassProducerConstr
#modules = [GenMassProducer()]
#interactive
#if len(sys.argv)==3:
#    w = float(sys.argv[2])
...
#    modules_ += [xsWeightProducer(w)]
    #from PhysicsTools.NanoAODTools.postprocessing.examples.SFProducer_ETau import SFProducerConstr_ETau
    ...
    #modules_ += [SFProducerConstr_ETau(year), SFProducerConstr_MuTau(year), SFProducerConstr_TauTau(year)]
...
if len(sys.argv)==4:
    ...
    from PhysicsTools.NanoAODTools.postprocessing.examples.xsWeightProducer import xsWeightProducer
    modules_ += [xsWeightProducer(w)]
    #from PhysicsTools.NanoAODTools.postprocessing.examples.SFProducer import SFProducerConstr
    #modules_ += [SFProducer(year)]
